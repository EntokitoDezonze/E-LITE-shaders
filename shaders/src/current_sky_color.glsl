...
float final_sun_factor2 = fastpow(sun_ss, dayBF(1.5, 0.0, 10.0));
    ...
    vec3 current_low_sky_color = mix(pure_mid_sky_color * dayBF(mix(dayBF(4.5, 4.5, 2.0), 2.0, rainStrength), mix(1.0, 0.25, rainStrength), mix(3.0, 2.0, rainStrength)) * 0.5 + low_sky_color * dayBF(0.1, 1.0, 0.05), low_sky_color * dayBFlgcy(2.0, 1.5, mix(1.5, 3.0, rainStrength)), final_sun_factor);
    vec3 current_mid_sky_color = mix((pure_hi_sky_color + pure_mid_sky_color) * dayBFlgcy(dayBF(0.4, 0.4, 0.0), mix(0.7, 0.2, rainStrength), mix(0.5, 0.4, rainStrength)) + (mid_sky_color * 2.0 * lightning), mid_sky_color * dayBF(1.0, 1.0, 0.75) + (mid_sky_color * 2.0 * lightning), final_sun_factor2);
    vec3 current_hi_sky_color = mix(hi_sky_color * dayBF(dayBF(0.8, 0.8, 0.0), 1.0, 1.0) + (hi_sky_color * lightning), hi_sky_color + (hi_sky_color * lightning), final_sun_factor);
#else
